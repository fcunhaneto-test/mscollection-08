<template>
    <table v-if="titles" class="table is-fullwidth">
        <thead>
        <tr>
            <th scope="col">Títulos</th>
            <th scope="col">Ano</th>
            <th v-if="table" scope="col">Tempo</th>
            <th scope="col">Classificação<br/>IMDB</th>
            <th scope="col">Classificação<br/>Editor<sup class="has-text-danger">[1]</sup></th>
            <th scope="col">Categoria 1</th>
            <th scope="col">Categoria 2</th>
            <th scope="col">Categoria 3</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(title, i) in titles" :key="title.id">
            <th scope="row"><a class="table-link" @click="toEdit(title)">{{ title.title }}</a></th>
            <td>{{ title.year }}</td>
            <td v-if="table">{{ title.time | strTime }}</td>
            <td>{{ title.imdb_rating}}</td>
            <td>
                <i v-for="r in title.our_rating" class="fas fa-star has-text-orange"></i>
                <i v-for="r in (5 - title.our_rating)" class="far fa-star has-text-orange"></i>
            </td>
            <td>{{ title.category_1 }}</td>
            <td>{{ title.category_2 }}</td>
            <td>{{ title.category_3 }}</td>
        </tr>
        </tbody>
    </table>
</template>

<script>
export default {
    name: "TitlesTable",
    props: {
        table: Number,
    },
    computed: {
        titles() {
            return this.$store.getters.getTitles
        }
    },
    methods: {
        toEdit(title) {
            this.$store.commit('SET_TITLE', title)
            if(this.table) {
                this.$router.push({name: 'edit-movie'})
            }
            this.$router.push({name: 'edit-series'})
        }
    }
}
</script>

<style scoped>

</style>
